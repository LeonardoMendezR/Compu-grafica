...
class HitBox(Hit):
    ...
    def check_hit(self, origin, direction):
        ...
        t1 = glm.min(tmin, tmax) # entrada del rayo
        t2 = glm.max(tmin, tmax) # salida del rayo
        
        t_near = glm.max(glm.max(t1.x, t1.y), t1.z) # valor de entrada global
        t_far = glm.min(glm.min(t2.x, t2.y), t2.z) # valor de salida global
...
class HitBoxOBB(Hit):
    ...
    def check_hit(self, origin, direction):
        ...
        local_dir = glm.vec3(inv_model * glm.vec4(direction, 0.0))
        ...
        t_near = max(t1.x, t1.y, t1.z) # valor de entrada global
        t_far = min(t2.x, t2.y, t2.z) # valor de salida global
